...
with st.sidebar:
    ...
    st.subheader("스타일 정보")
    st.session_state.user_info["style_preference"] = st.selectbox("선호 스타일", ["캐주얼", "미니멀", "스트릿", "포멀", "빈티지", "스포티"], index=["캐주얼", "미니멀", "스트릿", "포멀", "빈티지", "스포티"].index(st.session_state.user_info.get("style_preference", "캐주얼")))
    ...
    st.session_state.user_info["personal_color"] = st.selectbox("퍼스널 컬러", ["모름", "봄 웜톤", "여름 쿨톤", "가을 웜톤", "겨울 쿨톤"], index=["모름", "봄 웜톤", "여름 쿨톤", "가을 웜톤", "겨울 쿨톤"].index(st.session_state.user_info.get("personal_color", "모름")))
...
def get_kma_weather_forecast(coords, service_key, target_date):
    ...
    # 기상청 단기예보는 3일치 예보를 제공. target_date가 너무 먼 경우 API 호출이 실패할 수 있음.
    # base_date는 '오늘' 또는 '어제'여야 함.
    # target_date에 대한 예보를 얻으려면, base_date는 오늘 또는 어제여야 함.
    # API는 base_date+base_time 시점부터 3일간의 예보를 제공.
    # 가장 안정적인 방법은 base_date를 오늘로 설정하는 것.
    ...
    try:
        ...
        for item in items:
            if item.get("fcstDate") == target_date_str:
                ...
                if category:
                    if category not in target_day_weather:
                        target_day_weather[category] = []
...
if prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("선택하신 날짜의 날씨를 확인하고, 맞춤 스타일을 추천하는 중..."):
            ...
            user_info_text = (
                f"- 지역: {location_name}\n"
                f"- 날짜: {target_date.strftime('%Y년 %m월 %d일')}\n"
